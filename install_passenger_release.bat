@echo off
REM Build and install ONLY the passenger Release APK, then launch it on a connected device

cd /d "%~dp0"
echo ========================================
echo Build and Install: passenger Release
echo ========================================

setlocal EnableExtensions EnableDelayedExpansion

REM ------------------------------------------
REM  Detect a compatible JDK (17 or higher)
REM ------------------------------------------
...
REM ------------------------------------------
REM  Build passenger Release APK
REM ------------------------------------------
echo.
echo Building passenger Release APK...
call gradlew.bat assemblePassengerRelease --no-daemon
if %ERRORLEVEL% NEQ 0 (
    echo BUILD FAILED for passenger Release. See errors above.
    pause >nul
    exit /b 1
)

set "APK_PATH=app\build\outputs\apk\passenger\release\app-passenger-release.apk"
echo Checking for APK: %APK_PATH%
if not exist "%APK_PATH%" (
    echo ERROR: APK not found at expected path.
    dir "app\build\outputs\apk\passenger\release" 2>nul
    pause >nul
    exit /b 1
)
...
REM ------------------------------------------
REM  Check devices and pick first serial
REM ------------------------------------------
set "DEVICE_SERIAL="
...
REM ------------------------------------------
REM  Install APK (replace if exists)
REM ------------------------------------------
echo Installing passenger Release APK...
"%ADB%" -s "%DEVICE_SERIAL%" install -r "%APK_PATH%"
if %ERRORLEVEL% NEQ 0 (
    echo Install failed. Trying uninstall+install...
    "%ADB%" -s "%DEVICE_SERIAL%" uninstall com.example.toda.passenger >nul 2>&1
...
"%ADB%" -s "%DEVICE_SERIAL%" shell am start -n "com.example.toda.passenger/com.example.toda.MainActivity"

if %ERRORLEVEL% EQU 0 (
    echo Success! passenger Release installed and launched.
) else (
