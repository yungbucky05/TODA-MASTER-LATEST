...
async function handleCreateAccount(event) {
  ...
  try {
    ...
    // IMPORTANT: Use phone number format for Firebase Auth email to match mobile app login
    // The mobile app converts phone number to email format: {phoneNumber}@toda-admin.com
    const authEmail = `${normalizedPhone}@toda-admin.com`;
    let firebaseUser;
    ...
    try {
      ...
      if (authError.code === 'auth/email-already-in-use') {
        throw new Error("This phone number is already registered in Firebase Authentication.");
    ...
    // Step 2: Create user record in Realtime Database with Firebase Auth UID
    const hashedPassword = await hashPassword(password);
    const now = Date.now();
    const userId = firebaseUser.uid; // Use Firebase Auth UID instead of push key
    ...
    const accountData = {
      ...
      email: email || null, // Store the optional email separately
      firebaseAuthEmail: authEmail, // Store the Firebase Auth email (phone format)
      userType: role,
      role: role,
      isActive: true,
      isVerified: true,
    ...
    // Step 3: Create auth mapping for reference
